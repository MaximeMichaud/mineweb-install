...
function installMineWeb () {
	...
	if [[ "$OS" =~ (debian|ubuntu) ]]; then
		if [[ "$VERSION_ID" = "8" ]]; then
		    ...
		    tar xzf phpMyAdmin-4.9.3-all-languages.tar.gz
		    mv phpMyAdmin-4.9.3-all-languages/* /usr/share/phpmyadmin
		    rm /usr/share/phpmyadmin/phpMyAdmin-4.9.3-all-languages.tar.gz
		    rm -rf /usr/share/phpmyadmin/phpMyAdmin-4.9.3-all-languages
